...
Patient::Patient() : PID(0), requestTime(0), pickupTimes(0), HID(0), distance(0), severity(0), pType(0), waitingTime(0) {}

Patient::Patient(int _pType, int _PID, int _HID, int _distance, int _requestTime, int _severity) :
        pType(_pType), PID(_PID), HID(_HID), distance(_distance), requestTime(_requestTime), severity(_severity), pickupTimes(0), waitingTime(0) {}
